...
__export(index_exports, {
  ...
  bool64bitKeyCodec: () => bool64bitKeyCodec,
...
var StructEntry = class {
  ...
  hash = 0;
  readIndex;
  constructor(view, type, hash2, offset, readIndex) {
...
var GameDataRecord = class {
  ...
  addNew(hash2, value) {
    const entry = new StructEntry(scratchDataView, this.#structType, hash2, 0);
...
var GameData = class _GameData extends BaseGameData {
  ...
  static toArrayBuffer(gameData) {
    ...
    for (const type of structTypes) {
      ...
      const entries = Object.keys(section).map((key) => Number(key)).filter((key) => Number.isFinite(key)).map((hash2) => [hash2, section[hash2]]);
    ...
    for (const type of structTypes) {
      ...
      indexCursor += 8;
      for (const [hash2, entry] of entries) {
        view.setUint32(indexCursor, hash2, true);
