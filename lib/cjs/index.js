...
// src/GameDataLib.ts
var GameDataLib = class {
  static fromArrayBuffer(buffer) {
    const data = new DataView(buffer);
    if (data.byteLength < 32) {
      throw new ParseError("Invalid file. Too Small.");
    }
    if (data.getUint32(0, true) !== 16909060) {
      throw new Error("Invalid File. Magic Mismatch.");
    }
    const dataOffset = data.getUint32(8, true);
    if (dataOffset < 32 || dataOffset > data.byteLength) {
    ...
    while (offset + 8 <= dataOffset) {
      ...
      offset += 8;
      if (hashKey === 0) {
        const sectionType = hashObjectValue;
